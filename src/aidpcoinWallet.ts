...
export class Wallet {
  rpc = getRPC("anonymous", "anonymous", URL_AIDPCOIN_MAINNET);
  _mnemonic = "";
  network: ChainType = "aidp";
  addressObjects: Array<IAddressMetaData> = [];
  receiveAddress = "";
  changeAddress = "";
  addressPosition = 0;
  baseCurrency = "AIDP"; //Default is AIDP but it could be EVR
  offlineMode = false;
  ...
  async init(options: IOptions) {
    ...
    if (options.network === "aidp-test") {
      url = URL_AIDPCOIN_TESTNET;
    }
    ...
    const hdKey = AidpcoinKey.getHDKey(this.network, this._mnemonic);
    const coinType = AidpcoinKey.getCoinType(this.network);
    const ACCOUNT = 0;
    ...
    while (doneDerivingAddresses === false) {
      ...
      for (let i = 0; i < 20; i++) {
        const external = AidpcoinKey.getAddressByPath(
          this.network,
          hdKey,
          `m/44'/${coinType}'/${ACCOUNT}'/0/${this.addressPosition}`
        );

        const internal = AidpcoinKey.getAddressByPath(
          this.network,
          hdKey,
